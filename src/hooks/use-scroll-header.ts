...
interface ScrollHeaderState {
  /** Header is hidden (slid up) — mobile only, on scroll-down. */
  hidden: boolean;
  /** Header is in compact mode — slight height reduction + shadow, when scrolled. */
  compact: boolean;
}
 ...
 */
export function useScrollHeader(forceVisible = false): ScrollHeaderState {
