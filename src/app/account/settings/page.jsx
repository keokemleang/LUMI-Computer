...
import { User, Bell, Shield, Palette, Save, Check, Loader2, Sun, Package, Mail } from "lucide-react";
...
function NotificationsSection() {
  ...
  const items = [{
    ...
    icon: Mail,
    title: "Newsletter",
    description: "Monthly digest of new products and restocks.",
    checked: newsletter,
    onChange: v => {
      setNewsletter(v);
      ...
      <CardContent className="p-0">
        <ul className="divide-y divide-border">
          ...
          return <li key={item.key} className="flex items-center justify-between gap-4 px-6 py-4">
                ...
                <Switch checked={item.checked} onCheckedChange={item.onChange} aria-label={item.title} />
              </li>;
        })}
        </ul>
      </CardContent>
  ...
  return <div className="space-y-4">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <Input id="pwd-current" type="password" value={current} onChange={e => setCurrent(e.target.value)} autoComplete="current-password" />
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                <Label htmlFor="pwd-new">New password</Label>
                <Input id="pwd-new" type="password" value={next} onChange={e => setNext(e.target.value)} autoComplete="new-password" />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="pwd-confirm">Confirm new password</Label>
                <Input id="pwd-confirm" type="password" value={confirm} onChange={e => setConfirm(e.target.value)} autoComplete="new-password" />
              </div>
            </div>
      ...
      <Card className="gap-0">
        ...
        <CardContent className="flex items-center justify-between gap-4">
          ...
          <Button variant="outline" onClick={() => toast.info("2FA setup is a demo feature", {
          description: "Coming soon to LUMI Computer accounts."
        })}>
            Enable 2FA
          </Button>
        </CardContent>
...
function AppearanceSection() {
  return <Card className="gap-0">
      <CardHeader>
        ...
        <CardDescription>
          Customize how LUMI Computer looks on your device.
        </CardDescription>
      </CardHeader>
