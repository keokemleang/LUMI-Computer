...
export default function AddressesPage() {
  const [addresses, setAddresses] = React.useState<Address[]>([DEFAULT_ADDRESS]);
  const [dialogOpen, setDialogOpen] = React.useState(false);

  function handleAddAddress(addr: Omit<Address, "id">) {
    setAddresses((prev) => [
      ...prev,
  ...
  function handleRemove(id: string) {
    setAddresses((prev) => {
      ...
      // If we removed the default and others exist, promote the first.
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {addresses.map((addr) => {
          ...
          return (
            <Card key={addr.id} className="gap-0 p-5">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      toast.info("Editing coming soon", {
                        description: "Address editing is a demo feature.",
                      })
                    }
                  >
...
function AddAddressDialog({
  ...
  onSubmit: (addr: Omit<Address, "id">) => void;
}) {
  ...
  const [label, setLabel] = React.useState<"Home" | "Work" | "Other">("Home");
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    // Simulate async work for nicer UX
    window.setTimeout(() => {
      onSubmit({
        name: name.trim(),
        street: street.trim(),
  ...
  const trigger = asCard ? (
    <button
      type="button"
      className={cn(
        "flex h-full min-h-44 w-full flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-border bg-muted/20 p-6 text-sm font-medium text-muted-foreground transition-colors hover:border-primary hover:bg-accent hover:text-foreground"
      )}
    >
