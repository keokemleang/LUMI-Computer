...
import { ChevronRight, Package, FileDown, Truck, ShieldCheck, MessageSquare } from "lucide-react";
...
export default async function ProductDetailPage({
  ...
  const [related, reviews] = await Promise.all([getRelatedProducts(product.slug, product.categoryId), getReviews(product.id)]);
  const discount = product.compareAt ? Math.round((product.compareAt - product.price) / product.compareAt * 100) : 0;
  const inStock = product.stock > 0;
  const lowStock = product.stock > 0 && product.stock < 10;
  const stockBadge = inStock ? lowStock ? <Badge className="bg-warning text-warning-foreground">
        Low stock — only {product.stock} left
      </Badge> : <Badge className="bg-success text-success-foreground">In stock</Badge> : <Badge variant="destructive">Out of stock</Badge>;
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-2 lg:gap-12">
        ...
        <div className="flex flex-col">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3">
            ...
            <a href="#reviews" className="text-sm text-muted-foreground hover:text-primary hover:underline">
              Read reviews
            </a>
          </div>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            ...
            {product.compareAt && <span className="text-lg text-muted-foreground line-through">
          ...
          {/* Add to cart */}
          <AddToCart slug={product.slug} name={product.name} price={product.price} image={product.images[0]} stock={product.stock} />
      ...
      <div className="mt-12">
        <Tabs defaultValue="description" className="w-full">
          ...
          <TabsContent value="specs" className="mt-6">
            <Card className="p-6">
              {product.specs.length > 0 ? <>
                  ...
                  <Table>
                    <TableBody>
                      {product.specs.map((s, i) => <TableRow key={i}>
          ...
          <TabsContent value="downloads" className="mt-6">
            <Card className="p-6">
              {product.datasheetUrl ? <a href={product.datasheetUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between rounded-lg border border-border bg-muted/30 p-4 transition-colors hover:bg-accent">
            ...
            <Card className="p-6">
              {reviews.length > 0 ? <>
                  ...
                  <div className="space-y-6">
                    {reviews.map(r => <div key={r.id} className="border-b border-border pb-6 last:border-0 last:pb-0">
